...
function App() {
  const { start, stop, isRunning, aiText } = useRealtimeVoice()
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col items-center justify-between overflow-hidden bg-[#FFFCF8] px-6 py-12 font-sans text-stone-600 select-none">
      ...
      {/* Intro Text Area & AI Response Display */}
      {/* 修改布局：justify-center 稍微往下一点，改为 pt-20 pb-20 调整垂直重心 */}
      <div className="z-10 flex w-full max-w-md flex-1 flex-col items-center justify-center pt-20 pb-20">
        <AnimatePresence mode="wait">
          ...
          {isRunning && aiText && (
            <motion.div
              key="ai-text-container"
              className="flex min-h-[200px] w-full flex-col items-center px-6 text-center"
            >
      ...
      {/* Main Button Area */}
      <div
        className={`z-10 flex w-full flex-none items-center justify-center transition-all duration-1000 ease-in-out ${isRunning ? "pb-16" : "pb-20"}`}
      >
